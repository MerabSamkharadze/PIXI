...
@Component({
  selector: 'mg-blocks-game-over-modal',
  imports: [RouterLink],
  templateUrl: './game-over-modal.html',
  styleUrl: './game-over-modal.scss',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class BlocksGameOverModalComponent {
  ...
  protected readonly verdict = computed(() => {
    ...
    if (this.score() === 0) return 'Stack collapsed';
    return 'Stack maxed out';
  });
  ...
  protected readonly sparkles = Array.from({ length: 18 }, () => ({
    ...
    hue: ['pink', 'violet', 'amber'][Math.floor(Math.random() * 3)]
  }));
}
